...
async function sendMessage(event, context) {
  ...
  console.log('would send ' + JSON.stringify(event))

  var to_send = []
  event.Records.forEach(function (entry) {
    to_send.push(AWS.DynamoDB.Converter.unmarshall(entry.dynamodb.NewImage));
  });

  const body = {
    "name": "NewOrder",
    "channelId": "General",
    "content": JSON.stringify(to_send)
  }
